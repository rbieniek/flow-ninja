...
public class Netflow9PacketDecoderTest {
	...
	private static final byte[] dataFlowOnlyDatagram = new byte[] { 0x00, 0x09, // Version
																				// 9
	...
	public void decodeDataTemplateOnlyDatagram() {
		...
		final FlowRegistry flowRegistry = peerRegistry.registryForPeerAddress(LOCALHOST, 0);

		assertThat(flowRegistry.hasTemplateForFlowsetID(256)).isTrue();

		final Template template = flowRegistry.templateForFlowsetID(256);
	...
	public void decodeOptionsTemplateOnlyDatagram() {
		...
		final OptionsTemplate template = flowRegistry.optionTemplateForFlowsetID(260);
	...
	public void decodeDataTemplateOneDataFlowDatagram() throws Exception {
		...
		final DataFlow flow = (DataFlow) channel.inboundMessages().remove();
		...
		assertDataFlowRecord(it, FieldType.IPV4_SRC_ADDR,
				Inet4Address.getByAddress(new byte[] { (byte) 0xc0, (byte) 0xa8, 0x04, 0x0a }));
		assertDataFlowRecord(it, FieldType.IPV4_DST_ADDR,
		...
		assertDataFlowRecord(it, FieldType.SRC_TOS,
				EnumCodeValue.builder().value(IPTypeOfService.CS0).code(0x00).build());
		...
		assertDataFlowRecord(it, FieldType.IPV4_NEXT_HOP,
		...
		assertDataFlowRecord(it, FieldType.DIRECTION,
	...
	public void decodeDataTemplateTwoDataFlowDatagram() throws Exception {
		...
		assertDataFlowRecord(it, FieldType.IN_BYTES,
				CounterFactory.decode(new byte[] { 0x00, 0x00, 0x02, (byte) 0xfa }));
		assertDataFlowRecord(it, FieldType.IN_PKTS, CounterFactory.decode(new byte[] { 0x00, 0x00, 0x00, 0x01 }));
		...
		assertDataFlowRecord(it, FieldType.TCP_FLAGS, TCPFLags.class, TCPFLags.ACK);
		assertDataFlowRecord(it, FieldType.DIRECTION,
				EnumCodeValue.builder().value(FlowDirection.INGRESS).code(0x00).build());
		assertDataFlowRecord(it, FieldType.DST_AS, new Integer(0));
		assertDataFlowRecord(it, FieldType.SRC_AS, new Integer(0));
	...
	public void decodeOptionsTemplateAndOptionsDataDatagram() {
		channel.writeInbound(channel.alloc().buffer().writeBytes(optionsTemplateAndOptionsDataDatagram));
		...
		final OptionsFlow flow = (OptionsFlow) channel.inboundMessages().remove();
		final Header header = flow.getHeader();
	...
	public void decodeOptionsAndDataTemplateDatagram() {
		channel.writeInbound(channel.alloc().buffer().writeBytes(optionsAndDataTemplateDatagram));
		...
		final OptionsTemplate optionsTemplate = flowRegistry.optionTemplateForFlowsetID(260);
		...
		final Template dataTemplate = flowRegistry.templateForFlowsetID(256);
	...
	public void decodeDataAndOptionsTemplateOneDataFlowOneOptionFlowDatagram() throws Exception {
		channel.writeInbound(
				channel.alloc().buffer().writeBytes(dataAndOptionsTemplateOneDataFlowOneOptionFlowDatagram));
		...
		final DataFlow dataFlow = (DataFlow) channel.inboundMessages().remove();
		...
		assertDataFlowRecord(dataIt, FieldType.IPV4_SRC_ADDR,
				Inet4Address.getByAddress(new byte[] { (byte) 0xc0, (byte) 0xa8, 0x04, 0x0a }));
		assertDataFlowRecord(dataIt, FieldType.IPV4_DST_ADDR,
				Inet4Address.getByAddress(new byte[] { (byte) 0xc0, 0x2b, (byte) 0xac, 0x1e }));
		assertDataFlowRecord(dataIt, FieldType.PROTOCOL,
				EnumCodeValue.builder().value(IPProtocol.UDP).code(0x11).build());
		assertDataFlowRecord(dataIt, FieldType.SRC_TOS,
				EnumCodeValue.builder().value(IPTypeOfService.CS0).code(0x00).build());
		...
		assertDataFlowRecord(dataIt, FieldType.IPV4_NEXT_HOP,
				Inet4Address.getByAddress(new byte[] { (byte) 0xc0, (byte) 0xa8, 0x04, 0x04 }));
		...
		assertDataFlowRecord(dataIt, FieldType.DIRECTION,
				EnumCodeValue.builder().value(FlowDirection.EGRESS).code(0x01).build());
		...
		final OptionsFlow scopeFlow = (OptionsFlow) channel.inboundMessages().remove();
	...
	public void decodeDataFlowDatagramBeforeDataTemplate() throws Exception {
		channel.writeInbound(channel.alloc().buffer().writeBytes(dataFlowOnlyDatagram));
	...
	private void assertOptionsFlowRecord(final Iterator<OptionsFlowRecord> it, final FieldType type,
			final Object value) {
	...
	private <T> void assertDataFlowRecord(final Iterator<DataFlowRecord> it, final FieldType type, final Class<T> clazz,
			final T... values) {
		DataFlowRecord dfr;
