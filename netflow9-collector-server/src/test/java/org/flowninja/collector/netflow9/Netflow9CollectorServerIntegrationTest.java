...
public class Netflow9CollectorServerIntegrationTest {
	...
	public void shouldDecodeDataFlowWhenTemplateAndFlowInOneDatagram() throws Exception {
		completions.getServerEventCompletion().whenComplete((m, t) -> {
			if (m.isPresent()) {
				...
				try {
					final DatagramSocket socket = new DatagramSocket(SocketUtils.findAvailableUdpPort(49152),
							InetAddress.getLoopbackAddress());

					socket.send(
							new DatagramPacket(dataTemplateOneDataFlowDatagram, dataTemplateOneDataFlowDatagram.length,
									netflow9CollectorProperties.getAddress(), netflow9CollectorProperties.getPort()));
					socket.close();
				} catch (Exception e) {
		...
		assertThat(m.get().getDataFlow().getHeader()).isEqualTo(Header.builder().recordCount(2).sequenceNumber(31)
				.sourceId(0).sysUpTime(1976904).unixSeconds(1427139251L).build());
	...
	public static class TestConfiguration {
		@Bean
		public Netflow9CollectorProperties netflow9CollectorProperties() {
		...
		public SinkActorsProvider sinkActorsProvider(final SpringActorProducer springActorProducer,
			...
			return SinkActorsProvider.builder()
					.dataFlowActor(springActorProducer.createActor(SingleMessageTimedSinkActor.class,
							DataFlowMessage.class, completionHolder.getDataFlowCompletion(), 5))
					.optionsFlowActor(springActorProducer.createActor(SingleMessageTimedSinkActor.class,
							OptionsFlowMessage.class, completionHolder.getOptionsFlowCompletion(), 5))
					.decodingFailureActor(springActorProducer.createActor(SingleMessageTimedSinkActor.class,
							TemplateDecodingFailureMessage.class, completionHolder.getDecodingFailureCompletion(), 5))
					.serverEventActor(springActorProducer.createActor(SingleMessageTimedSinkActor.class,
							NetworkServerStartupMessage.class, completionHolder.getServerEventCompletion(), 5))
					.build();
		}
	}
