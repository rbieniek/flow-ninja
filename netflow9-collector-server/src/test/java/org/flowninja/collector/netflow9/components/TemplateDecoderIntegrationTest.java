			...
			if (msg instanceof Netflow9DecodedDatagram) {
				...
				decoded.getFlows().forEach(fb -> {
					...
					if (dataTemplates.containsKey(flowsetId)) {
						dataFlows.addAll(templateDecoder.decodeDataTemplate(decoded.getPeerAddress(), fb,
								dataTemplates.get(flowsetId)));
					} else if (optionsTemplates.containsKey(flowsetId)) {
						optionsFlows.addAll(templateDecoder.decodeOptionsTemplate(decoded.getPeerAddress(), fb,
								optionsTemplates.get(flowsetId)));
					} else {
