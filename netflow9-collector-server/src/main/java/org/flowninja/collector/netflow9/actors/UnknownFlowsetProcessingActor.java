...
public class UnknownFlowsetProcessingActor extends UntypedActor {
				...
				.onType(TemplateRegistryActor.DataTemplateAvailableRequest.class, m -> {
					getFlowBuffers(m.getDataTemplate().getFlowsetId()).forEach(p -> {
						templateDecoderActor.tell(TemplateDecoderActor.DataTemplateDecoderRequest.builder()
								.dataTemplate(m.getDataTemplate()).peerAddress(p.getLeft()).flowBuffer(p.getRight())
								.build(), getSelf());
					});
					...
					getFlowBuffers(m.getOptionsTemplate().getFlowsetId()).forEach(p -> {
						templateDecoderActor.tell(TemplateDecoderActor.OptionsTemplateDecoderRequest.builder()
								.optionsTemplate(m.getOptionsTemplate()).peerAddress(p.getLeft())
								.flowBuffer(p.getRight()).build(), getSelf());
					});
	...
	private List<Pair<InetAddress, FlowBuffer>> getFlowBuffers(final int flowsetId) {
		...
		return flowBufferMap.remove(flowsetId);
	}
